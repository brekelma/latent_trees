...
if useDistances
    ...
    %distance = distances(samples);
    prob_bij = computeBnStats(samples);
    assignin('base', 'prob_bij', computeBnStats(samples));
...
% IS THIS A PROBLEM IF NOT TREE STRUCTURED?
...
for j=1:length(internal_nodes)
    ...
    [adjmatS, edge_distanceS] = RG(distance(i_family,i_family), 1, numSamples, min_max);
